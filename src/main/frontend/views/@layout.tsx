...
function Header() {
  ...
  return (
    <div className="flex p-m gap-m items-center" slot="drawer">
      ...
      <span className="font-semibold text-l">Practlogin</span>
    </div>
  );
}
  ...
  const items = [
    {
      ...
      children:[
        {text: 'Perfil Personal', action: () =>console.log('View Profile')},
        {text: 'Cerrar sesion', action: () =>(async() => CuentaService.longout().then(async function(){
                                await logout();
          }))()},
...
//AQUIIIII MODIFIQUE todo el sistema necesita que este iniciado sesion
